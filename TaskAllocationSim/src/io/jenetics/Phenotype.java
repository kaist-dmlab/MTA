 ...
 * environment where the {@link Genotype} lives. This class implements the
 * {@link Comparable} interface, to define a natural order between two
 * {@code Phenotype}s. The natural order of the {@code Phenotypes} is defined by
 * its fitness value (given by the fitness {@link Function}. The
 * {@code Phenotype} is immutable and therefore can't be changed after creation.
 * <p>
 * The evaluation of the fitness function is performed lazily. Either by calling
 * one of the fitness accessors ({@link #getFitness()} or
 * {@link #getRawFitness()}) of through the <i>evaluation</i> methods
 * {@link #run()} or {@link #evaluate()}. Since the {@code Phenotype} implements
 * the {@link Runnable} interface, it is easily possible to perform the fitness
 * function evaluation concurrently, by putting it into an
...
public final class Phenotype<G extends Gene<?, G>, C extends Comparable<? super C>>
	 ...
	 *
	 * @param genotype
	 *            the genotype of this phenotype.
	 * @param generation
	 *            the current generation of the generated phenotype.
	 * @param function
	 *            the fitness function of this phenotype.
	 * @param scaler
	 *            the fitness scaler.
	 * @throws NullPointerException
	 *             if one of the arguments is {@code null}.
	 * @throws IllegalArgumentException
	 *             if the given {@code generation} is {@code < 0}.
	 */
	private Phenotype(final Genotype<G> genotype, final long generation,
	...
	/**
	 * Evaluates the (raw) fitness values and caches it so the fitness
	...
	/**
	 * Return the fitness function used by this phenotype to calculate the (raw)
	 * fitness value.
	 *
	...
	/**
	 * Return the age of this phenotype depending on the given current
	 * generation.
	 *
	 * @param currentGeneration
	 *            the current generation evaluated by the GA.
	...
	public boolean equals(final Object obj) {
		return obj instanceof Phenotype<?, ?>
				&& Objects.equals(getFitness(),
						((Phenotype<?, ?>) obj).getFitness())
				&& Objects.equals(getRawFitness(),
						((Phenotype<?, ?>) obj).getRawFitness())
	 ...
	 * @throws NullPointerException
	 *             if the given {@code genotype} is {@code null}.
	 */
	 ...
	 *
	 * @param genotype
	 *            the new genotype of the new phenotype.
	 * @param generation
	 *            date of birth (generation) of the new phenotype.
	 * @return New {@link Phenotype} with the same fitness {@link Function}.
	 * @throws NullPointerException
	 *             if the {@code genotype} is {@code null}.
	 */
	public Phenotype<G, C> newInstance(final Genotype<G> genotype,
	...
	/**
	 * Return a new phenotype with the the genotype of this and with new fitness
	 * function, fitness scaler and generation.
	 *
	 * @param generation
	 *            the generation of the new phenotype.
	 * @param function
	 *            the (new) fitness scaler of the created phenotype.
	 * @param scaler
	 *            the (new) fitness scaler of the created phenotype
	 * @return a new phenotype with the given values.
	 * @throws NullPointerException
	 *             if one of the values is {@code null}.
	 * @throws IllegalArgumentException
	 *             if the given {@code generation} is {@code < 0}.
	 */
	...
	/**
	 * The {@code Genotype} is copied to guarantee an immutable class. Only the
	 * age of the {@code Phenotype} can be incremented.
	 *
	 * @param <G>
	 *            the gene type of the chromosome
	 * @param <C>
	 ...
	 *             if the given {@code generation} is {@code < 0}.
	 */
	public static <G extends Gene<?, G>, C extends Comparable<? super C>> Phenotype<G, C> of(
			final Genotype<G> genotype, final long generation,
		...
		return Phenotype
				.<G, C> of(
						...
						generation,
						function,
						function instanceof Serializable ? (Function<? super C, ? extends C> & Serializable) a -> a
								: a -> a);
	}
	...
	public static <G extends Gene<?, G>, C extends Comparable<? super C>> Phenotype<G, C> of(
			...
			final Function<? super Genotype<G>, ? extends C> function,
			final Function<? super C, ? extends C> scaler) {
		return new Phenotype<>(genotype, generation, function, scaler);
	}
