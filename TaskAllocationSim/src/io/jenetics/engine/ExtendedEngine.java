...
public class ExtendedEngine<G extends Gene<?, G>, C extends Comparable<? super C>>
	...
	private EngineListener<G, C> engineListener;

	/**
	 * Create a new GA engine with the given parameters.
	 *
	 * @param fitnessFunction
	 *            the fitness function this GA is using.
	 * @param genotypeFactory
	 *            the genotype factory this GA is working with.
	 * @param fitnessScaler
	 *            the fitness scaler this GA is using.
	 * @param survivorsSelector
	 *            the selector used for selecting the survivors
	 * @param offspringSelector
	 *            the selector used for selecting the offspring
	 * @param alterer
	 *            the alterer used for altering the offspring
	 * @param validator
	 *            phenotype validator which can override the default
	 *            implementation the {@link Phenotype#isValid()} method.
	 * @param optimize
	 *            the kind of optimization (minimize or maximize)
	 * @param offspringCount
	 *            the number of the offspring individuals
	 * @param survivorsCount
	 *            the number of the survivor individuals
	 * @param maximalPhenotypeAge
	 *            the maximal age of an individual
	 * @param executor
	 *            the executor used for executing the single evolve steps
	 * @param clock
	 *            the clock used for calculating the timing results
	 * @param individualCreationRetries
	 *            the maximal number of attempts for creating a valid
	 *            individual.
	 * @throws NullPointerException
	 *             if one of the arguments is {@code null}
	 * @throws IllegalArgumentException
	 *             if the given integer values are smaller than one.
	 */
	protected ExtendedEngine(
			...
			final Optimize optimize, final int offspringCount,
			final int survivorsCount, final long maximalPhenotypeAge,
			final Executor executor, final Clock clock,
			final int individualCreationRetries,
			final UnaryOperator<EvolutionResult<G, C>> mapper,
			EngineListener<G, C> engineListener) {
	 ...
	 * @param generation
	 *            the current generation; used for calculating the phenotype
	 *            age.
	 * @return the evolution result
	 * @throws java.lang.NullPointerException
	 *             if the given {@code population} is {@code null}
	 * @throws IllegalArgumentException
	 *             if the given {@code generation} is smaller then one
	 */
	public EvolutionResult<G, C> evolve(final ISeq<Phenotype<G, C>> population,
	 ...
	 *
	 * @param start
	 *            the evolution start object
	 * @return the evolution result
	 * @throws java.lang.NullPointerException
	 *             if the given evolution {@code start} is {@code null}
	 */
	public EvolutionResult<G, C> evolve(final EvolutionStart<G, C> start) {
		final Timer timer = Timer.of(_clock).start();

		// 초기 Population 에 대해 적합도 계산
		final Timer evaluateTimer = Timer.of(_clock).start();
		ISeq<Phenotype<G, C>> initialPop = start.getPopulation();
		if (engineListener != null) {
			engineListener.beforeFitnessEvaluation(initialPop);
		}
		evaluate(initialPop);
		if (engineListener != null) {
			engineListener.afterFitnessEvaluation(initialPop);
		}
		evaluateTimer.stop();

		// Select the offspring population.
		final CompletableFuture<TimedResult<ISeq<Phenotype<G, C>>>> offspring = _executor
				.async(() -> selectOffspring(start.getPopulation()), _clock);

		// Select the survivor population.
		final CompletableFuture<TimedResult<ISeq<Phenotype<G, C>>>> survivors = _executor
				.async(() -> selectSurvivors(start.getPopulation()), _clock);

		// Altering the offspring population.
		final CompletableFuture<TimedResult<AltererResult<G, C>>> alteredOffspring = _executor
				.thenApply(offspring,
						p -> _alterer.alter(p.result, start.getGeneration()),
						_clock);

		// Filter and replace invalid and old survivor individuals.
		final CompletableFuture<TimedResult<FilterResult<G, C>>> filteredSurvivors = _executor
				.thenApply(survivors,
						pop -> filter(pop.result, start.getGeneration()),
						_clock);

		// Filter and replace invalid and old offspring individuals.
		final CompletableFuture<TimedResult<FilterResult<G, C>>> filteredOffspring = _executor
				.thenApply(
						alteredOffspring,
						pop -> filter(pop.result.getPopulation(),
								start.getGeneration()), _clock);

		// Combining survivors and offspring to the new population.
		final CompletableFuture<ISeq<Phenotype<G, C>>> population = filteredSurvivors
				.thenCombineAsync(filteredOffspring, (s, o) -> ISeq
						.of(s.result.population.append(o.result.population)),
						_executor.get());

		// Survivor 와 Offspring 을 합친 Population 결과 받아오기
		final ISeq<Phenotype<G, C>> combinedPop = population.join();

		// 적합도 계산 안된 새로운 Phenotype 에 대해 적합도 계산
		if (engineListener != null) {
			engineListener.beforeFitnessEvaluation(combinedPop);
		}
		final TimedResult<ISeq<Phenotype<G, C>>> result = TimedResult.of(
				() -> evaluate(combinedPop), _clock).get();
		if (engineListener != null) {
			engineListener.afterFitnessEvaluation(combinedPop);
		}

		final EvolutionDurations durations = EvolutionDurations.of(offspring
				.join().duration, survivors.join().duration, alteredOffspring
				.join().duration, filteredOffspring.join().duration,
				filteredSurvivors.join().duration, result.duration
						.plus(evaluateTimer.getTime()), timer.stop().getTime());
		...
		EvolutionResult<G, C> evolutionResult = _mapper.apply(EvolutionResult
				.of(_optimize, result.result, start.getGeneration(), durations,
						killCount, invalidCount,
						alteredOffspring.join().result.getAlterations()));
		if (engineListener != null) {
			engineListener.afterEvolve(evolutionResult.getBestPhenotype());
		}
	...
	private ISeq<Phenotype<G, C>> selectSurvivors(
			final ISeq<Phenotype<G, C>> population) {
		return _survivorsCount > 0 ? _survivorsSelector.select(population,
				_survivorsCount, _optimize) : ISeq.empty();
	}

	// Selects the offspring population. A new population object is returned.
	private ISeq<Phenotype<G, C>> selectOffspring(
			final ISeq<Phenotype<G, C>> population) {
		return _offspringCount > 0 ? _offspringSelector.select(population,
				_offspringCount, _optimize) : ISeq.empty();
	}

	// Filters out invalid and old individuals. Filtering is done in place.
	private FilterResult<G, C> filter(final Seq<Phenotype<G, C>> population,
			final long generation) {
		int killCount = 0;
		int invalidCount = 0;
	...
	private Phenotype<G, C> newPhenotype(final long generation) {
		...
		do {
			phenotype = Phenotype.of(_genotypeFactory.newInstance(),
					generation, _fitnessFunction, _fitnessScaler);
	...
	private ISeq<Phenotype<G, C>> evaluate(
			final ISeq<Phenotype<G, C>> population) {
	...
	public Iterator<EvolutionResult<G, C>> iterator() {
		return new EvolutionIterator<>(this::evolutionStart, this::evolve);
	}
	...
	private EvolutionStart<G, C> evolutionStart() {
		...
		final ISeq<Phenotype<G, C>> population = MSeq
				.<Phenotype<G, C>> ofLength(size)
				.fill(() -> newPhenotype(generation)).toISeq();
	 ...
	 * @return a new <b>infinite</b> evolution iterator
	 * @throws java.lang.NullPointerException
	 *             if the given {@code genotypes} is {@code null}.
	 */
	public Iterator<EvolutionResult<G, C>> iterator(
			final Iterable<Genotype<G>> genotypes) {
		requireNonNull(genotypes);

		return new EvolutionIterator<>(() -> evolutionStart(genotypes, 1),
				this::evolve);
	}
	 ...
	 *
	 * @param genotypes
	 *            the initial individuals used for the evolution stream. Missing
	 *            individuals are created and individuals not needed are
	 *            skipped.
	 * @return a new evolution stream.
	 * @throws java.lang.NullPointerException
	 *             if the given {@code genotypes} is {@code null}.
	...
	public EvolutionStream<G, C> stream(final Iterable<Genotype<G>> genotypes) {
		...
		return EvolutionStream.of(() -> evolutionStart(genotypes, 1),
				this::evolve);
	}
	...
	private EvolutionStart<G, C> evolutionStart(
		...
		final Stream<Phenotype<G, C>> stream = Stream.concat(
				StreamSupport.stream(genotypes.spliterator(), false).map(
						gt -> Phenotype.of(gt, generation, _fitnessFunction,
								_fitnessScaler)), Stream
						.generate(() -> newPhenotype(generation)));

		final ISeq<Phenotype<G, C>> population = stream.limit(
				getPopulationSize()).collect(ISeq.toISeq());
	 ...
	 *            skipped.
	 * @param generation
	 *            the generation the stream starts from; must be greater than
	 *            zero.
	 * @return a new <b>infinite</b> evolution iterator
	 * @throws java.lang.NullPointerException
	 *             if the given {@code genotypes} is {@code null}.
	...
	public Iterator<EvolutionResult<G, C>> iterator(
		...
		return new EvolutionIterator<>(() -> evolutionStart(genotypes,
				generation), this::evolve);
	}
	...
	public EvolutionStream<G, C> stream(final Iterable<Genotype<G>> genotypes,
		...
		return EvolutionStream.of(() -> evolutionStart(genotypes, generation),
				this::evolve);
	}
	...
	public Iterator<EvolutionResult<G, C>> iterator(
		...
		return new EvolutionIterator<>(() -> evolutionStart(population, 1),
				this::evolve);
	}
	 ...
	 *             if the given {@code population} is {@code null}.
	 */
	public EvolutionStream<G, C> stream(final ISeq<Phenotype<G, C>> population) {
		requireNonNull(population);

		return EvolutionStream.of(() -> evolutionStart(population, 1),
				this::evolve);
	}

	private EvolutionStart<G, C> evolutionStart(
			final Seq<Phenotype<G, C>> population, final long generation) {
		final Stream<Phenotype<G, C>> stream = Stream.concat(
				population.stream().map(
						p -> p.newInstance(p.getGeneration(), _fitnessFunction,
								_fitnessScaler)), Stream
						.generate(() -> newPhenotype(generation)));

		final ISeq<Phenotype<G, C>> pop = stream.limit(getPopulationSize())
				.collect(ISeq.toISeq());
	 ...
	 *            skipped.
	 * @param generation
	 *            the generation the iterator starts from; must be greater than
	 *            zero.
	 * @return a new <b>infinite</b> evolution iterator
	 * @throws java.lang.NullPointerException
	 *             if the given {@code population} is {@code null}.
	 ...
	 */
	public Iterator<EvolutionResult<G, C>> iterator(
			final ISeq<Phenotype<G, C>> population, final long generation) {
		requireNonNull(population);
		require.positive(generation);

		return new EvolutionIterator<>(() -> evolutionStart(population,
				generation), this::evolve);
	}
	...
	public EvolutionStream<G, C> stream(final ISeq<Phenotype<G, C>> population,
		...
		return EvolutionStream.of(() -> evolutionStart(population, generation),
				this::evolve);
	}
	 ...
	 *
	 * @param result
	 *            the previously evolved {@code EvolutionResult}
	 * @return a new evolution stream, which continues a previous one
	 * @throws NullPointerException
	 *             if the given evolution {@code result} is {@code null}
	 */
	public Iterator<EvolutionResult<G, C>> iterator(
	 ...
	 * {@link EvolutionResult}. The stream is initialized with the population of
	 * the given {@code result} and its total generation
	 * {@link EvolutionResult#getTotalGenerations()}.
	 *
	 * <pre>
	 * {@code
	 ...
	 * );
	 * 
	 * private static final `ExtendedEngine<DoubleGene, Double>
	 * ENGINE = `ExtendedEngine.builder(PROBLEM)
	...
	/**
	 * Return the maximal number of attempt before the {@code `ExtendedEngine}
	 * gives up creating a valid individual ( {@code Phenotype}).
	 *
	 * @since 4.0
	 *
	...
	/**
	 * Create a new evolution {@code `ExtendedEngine.Builder} initialized with
	 * the values of the current evolution {@code `ExtendedEngine}. With this
	 * method, the evolution engine can serve as a template for a new one.
	 *
	...
	public Builder<G, C> builder() {
		return new Builder<G, C>(_genotypeFactory, _fitnessFunction)
				...
				.offspringFraction(
						(double) _offspringCount / (double) getPopulationSize())
				.offspringSelector(_offspringSelector).optimize(_optimize)
	...
	/**
	 * Create a new evolution {@code `ExtendedEngine.Builder} for the given
	 * {@link Problem}.
	 *
	 * @since 3.4
	 *
	 * @param problem
	 *            the problem to be solved by the evolution
	 *            {@code `ExtendedEngine}
	 * @param <T>
	 *            the (<i>native</i>) argument type of the problem fitness
	 *            function
	 * @param <G>
	 *            the gene type the evolution engine is working with
	 * @param <C>
	 *            the result type of the fitness function
	 * @return Create a new evolution {@code `ExtendedEngine.Builder}
	 */
	...
	/**
	 * Create a new evolution {@code `ExtendedEngine.Builder} with the given
	 * fitness function and genotype factory.
	 *
	 * @param ff
	...
	/**
	 * Create a new evolution {@code `ExtendedEngine.Builder} with the given
	 * fitness function and chromosome templates.
	 *
	 * @param ff
	 *            the fitness function
	 * @param chromosome
	...
	public static <G extends Gene<?, G>, C extends Comparable<? super C>> Builder<G, C> builder(
			final Function<? super Genotype<G>, ? extends C> ff,
			final Chromosome<G> chromosome, final Chromosome<G>... chromosomes) {
		return new Builder<>(Genotype.of(chromosome, chromosomes), ff);
	}

	/**
	 * Create a new evolution {@code `ExtendedEngine.Builder} with the given
	 * fitness function and problem {@code codec}.
	 *
	 * @since 3.2
	 *
	 * @param ff
	 ...
	 *            the problem codec
	 * @param <T>
	 *            the fitness function input type
	 * @param <C>
	 *            the fitness function result type
	 * @param <G>
	 *            the gene type
	...
	/**
	 * Builder class for building GA {@code `ExtendedEngine} instances.
	 *
	 * @see ExtendedEngine
	 *
	...
	public static final class Builder<G extends Gene<?, G>, C extends Comparable<? super C>>
		...
		private Alterer<G, C> _alterer = Alterer.of(
				new SinglePointCrossover<G, C>(0.2), new Mutator<>(0.15));
		...
		// `ExtendedEngine execution environment.
		...
		private Builder(final Factory<Genotype<G>> genotypeFactory,
				final Function<? super Genotype<G>, ? extends C> fitnessFunction) {
			_genotypeFactory = requireNonNull(genotypeFactory);
			_fitnessFunction = requireNonNull(fitnessFunction);
		}
		...
		/**
		 * Set the fitness function of the evolution {@code `ExtendedEngine}.
		 *
		 * @param function
		 *            the fitness function to use in the GA
		 *            {@code `ExtendedEngine}
		...
		/**
		 * Set the fitness scaler of the evolution {@code `ExtendedEngine}.
		 * <i>Default value is set to the identity function.</i>
		 *
		 * @param scaler
		 *            the fitness scale to use in the GA {@code `ExtendedEngine}
		 * @return {@code this} builder, for command chaining
		 */
		 ...
		 *
		 * @param genotypeFactory
		 *            the genotype factory for creating new individuals.
		 * @return {@code this} builder, for command chaining
		 */
		 ...
		 *
		 * @param selector
		 *            used for selecting survivors population
		 * @return {@code this} builder, for command chaining
		 */
		 ...
		 *
		 * @param selector
		 *            used for selecting survivors and offspring population
		 * @return {@code this} builder, for command chaining
		 */
		 ...
		 *
		 * @param first
		 *            the first alterer used for alter the offspring population
		 * @param rest
		 *            the rest of the alterers used for alter the offspring
		 *            population
		 * @return {@code this} builder, for command chaining
		 * @throws java.lang.NullPointerException
		 *             if one of the alterers is {@code null}.
		 */
		@SafeVarargs
		public final Builder<G, C> alterers(final Alterer<G, C> first,
				final Alterer<G, C>... rest) {
			requireNonNull(first);
			Stream.of(rest).forEach(Objects::requireNonNull);

			_alterer = rest.length == 0 ? first : Alterer.of(rest).compose(
					first);
		 ...
		 * {@link #genotypeFactory(Factory)}, which will replace any previously
		 * set phenotype validators.
		 *
		 * <p>
		 * <i>Default value is set to {@code Phenotype::isValid}.</i>
		 * </p>
		 *
		 * @since 3.1
		 *
		 ...
		 *
		 * @param validator
		 *            the {@code validator} used for validating the individuals
		 *            (phenotypes).
		 * @return {@code this} builder, for command chaining
		 * @throws java.lang.NullPointerException
		 *             if the {@code validator} is {@code null}.
		 */
		 ...
		 *
		 * <p>
		 * <i>Default value is set to {@code Genotype::isValid}.</i>
		 * </p>
		 *
		 * @since 3.1
		 *
		 ...
		 * @return {@code this} builder, for command chaining
		 * @throws java.lang.IllegalArgumentException
		 *             if the size is not within the range [0, population-size].
		 */
		public Builder<G, C> offspringSize(final int size) {
			if (size < 0) {
				throw new IllegalArgumentException(
						format("Offspring size must be greater or equal zero, but was %s.",
								size));
			}
		...
		public Builder<G, C> populationSize(final int size) {
			if (size < 1) {
				throw new IllegalArgumentException(
						format("Population size must be greater than zero, but was %s.",
								size));
			}
		...
		public Builder<G, C> maximalPhenotypeAge(final long age) {
			if (age < 1) {
				throw new IllegalArgumentException(format(
						"Phenotype age must be greater than one, but was %s.",
						age));
			}
		...
		/**
		 * The maximal number of attempt before the {@code `ExtendedEngine}
		 * gives up creating a valid individual ({@code Phenotype}). <i>Default
		 * values is set to {@code 10}.</i>
		 *
		 * @since 3.1
		 *
		 * @param retries
		 ...
		 * @throws IllegalArgumentException
		 *             if the given retry {@code count} is smaller than zero.
		 * @return {@code this} builder, for command chaining
		 */
		public Builder<G, C> individualCreationRetries(final int retries) {
			if (retries < 0) {
				throw new IllegalArgumentException(format(
						"Retry count must not be negative: %d", retries));
			}
		 ...
		 * @return {@code this} builder, for command chaining
		 * @throws NullPointerException
		 *             if the given {@code resultMapper} is {@code null}
		 */
		public Builder<G, C> mapping(
				final Function<? super EvolutionResult<G, C>, EvolutionResult<G, C>> mapper) {
			_mapper = requireNonNull(mapper::apply);
			return this;
		}

		/**
		 * Builds an new {@code `ExtendedEngine} instance from the set
		 * properties.
		 * 
		 * @param tpp
		 * @param jp
		 ...
		 *            `ExtendedEngine
		 *
		 * @return an new {@code `ExtendedEngine} instance from the set
		 *         properties
		 */
		public ExtendedEngine<G, C> build(EngineListener<G, C> engineListener) {
			return new ExtendedEngine<>(_fitnessFunction, _genotypeFactory,
					_fitnessScaler, _survivorsSelector, _offspringSelector,
					_alterer, _validator, _optimize, getOffspringCount(),
					getSurvivorsCount(), _maximalPhenotypeAge, _executor,
					_clock, _individualCreationRetries, _mapper, engineListener);
		}
		...
		/**
		 * Return the {@link Clock} the engine is using for measuring the
		 * execution time.
		 *
		 * @since 3.1
		 *
		...
		/**
		 * Return the used genotype {@link Factory} of the GA. The genotype
		 * factory is used for creating the initial population and new, random
		 * individuals when needed (as replacement for invalid and/or died
		 * genotypes).
		 *
		 * @since 3.1
		 *
		 ...
		 * {@code `ExtendedEngine} gives up creating a valid individual (
		 * {@code Phenotype}).
		 *
		 * @since 3.1
		 *
		...
		public Builder<G, C> copy() {
			return new Builder<G, C>(_genotypeFactory, _fitnessFunction)
					.alterers(_alterer).clock(_clock).executor(_executor)
					.fitnessScaler(_fitnessScaler)
					.maximalPhenotypeAge(_maximalPhenotypeAge)
					.offspringFraction(_offspringFraction)
					.offspringSelector(_offspringSelector)
					.phenotypeValidator(_validator).optimize(_optimize)
